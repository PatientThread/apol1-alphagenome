...
> **Superseded in part, 20 September 2026.** The reconciliation of 19-20 September
> overturned a third result: the beta-globin control locus is NOT null. Rebuilt on
> the same kidney-plus-podocyte reference used at the test locus it gives an odds
> ratio of 7.54 (3.66-15.31) against 8.76 at APOL1-MYH9, so the pipeline is not
> locus-specific. Draft 5 (`publication/MANUSCRIPT.md`) is built on that, as a
> BMC Research Notes research note reporting three results overturned by one error
> class: mismatching the measured evaluation set to the predicted track. Sections
> below that assert locus specificity, or that the tissue label carries no
> information, are kept as the record of what was believed when, and are not the
> current claim. Table 1 of draft 5, regenerated by `analysis/18_table_one.py`
> under a single merge and dedup convention, is the authoritative summary.
